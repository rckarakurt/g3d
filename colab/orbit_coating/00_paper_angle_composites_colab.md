## 8. Makale — 7 sentetik composite (polyp mucosa uzerine)

Unity mucosa + StyleShot polyp **tek goruntude birlestirilir** (polyp ortaya yapistirilir).
Unity ve polyp **ayri panel olarak gosterilmez**.

### Sabit eslestirme

| Unity | Polyp acisi |
|-------|-------------|
| image_0067 | -45° |
| image_0078 | -30° |
| image_0087 | -15° |
| image_0098 | 0° |
| image_0121 | +15° |
| image_0132 | +30° |
| image_0143 | +45° |

### Cikti

```
/content/paper_angle_composites/
├── singles/composite_f0067_azm045.png ...   (7 bindirilmis sentetik PNG)
├── paper_composites_strip_7.png           (7 composite yan yana — makale)
└── paper_composites_manifest.json
```

Once: **0** (git pull) → **5b** → **3** → **3c** → **8**
